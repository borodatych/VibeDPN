...
- [ ] NodeUI 4449 и `core` API доступны с домашней стороны через wg0 и больше ниоткуда; управление
      пирами (`POST/DELETE /peers`, `GET /peers/{name}/config` с закрытыми ключами) остаётся только
      на loopback VPS — через wg0 наружу лишь разрешённое чтение, иначе любая коробка заберёт чужие ключи
