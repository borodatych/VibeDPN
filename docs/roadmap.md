...
- [ ] Режим `full` на одном порту: IPv6 устройств идёт через роутер провайдера мимо коробки (RA не её) —
      AdGuard отдаёт в `full` пустые AAAA, `doctor` предупреждает о глобальном IPv6 в LAN (ревью роутера 2026-09-13)
