...
- [x] **`vibedpn init`** — ✅ (2026-09-12, `next`) детект интерфейса с default route, адреса и подсети
      ...
      прогоняется в CI-тесте установки
...
> **Решение владельца 2026-09-12:** UI должен быть человеко-понятным мастером настройки, стек —
> оценить start0 (Point0 + Prisma + better-auth) по глобальному правилу о старт-паке; отдельный
> план до начала стадии. Минимум функций: выбор входящего и исходящего интерфейса для одной коробки.

- [ ] План стека UI: start0 против SPA на React из §4 idea.md — с обоснованием и стоимостью на Pi
- [ ] Каркас UI, экран входа (пароль — тот же, что у `/api/` со Stage 4), слушает только LAN
