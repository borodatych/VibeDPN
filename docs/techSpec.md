...
| `hostapd` | `wifi` | host, `NET_ADMIN`, `NET_RAW` | образ `ghcr.io/borodatych/vibedpn-hostapd` (`images/hostapd`, Alpine 3.24); точка доступа на радио `lan_interface` при `network.wifi`; `hostapd.conf` с паролем из `secrets/wifi-passphrase` рендерит core в `data/hostapd` (600), контейнер монтирует каталог только на чтение; управляющий сокет `ctrl_interface=/var/run/hostapd` — это каталог `/run/vibedpn/hostapd` хоста, общий с `core`, который читает из него события и клиентов точки (решение 24) |
