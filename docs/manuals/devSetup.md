...
`doctor` и AdGuard, а также сервер доступа: человек по ссылке коробки выходит по её маршрутизации —
через VPS в `full`, напрямую в `off`, по правилу сети в `smart`, — петля коробки и LAN ему закрыты,
люди добавляются и убираются без перезапуска сервера, трафик считается. «Интернет» стенда — своя
