...
(через VPS или напрямую), kill-switch и `failopen` (правкой файла и `vibedpn up`, без `restart`),
недоступность сети шлюзов из LAN, `vibedpn mode`, `doctor` и AdGuard, новый файл выхода WireGuard
и новая ссылка `xray` (шлюз пересоздан и выходит через новое), а также сервер доступа: человек
по ссылке коробки выходит по её маршрутизации — через VPS в `full`, напрямую в `off`, по правилу
сети в `smart`, — петля коробки и LAN ему закрыты, люди добавляются и убираются без перезапуска
сервера, трафик считается. «Интернет» стенда — своя
Docker-сеть 198.18.0.0/24 с веб-сервером, который отвечает адресом клиента; настоящий интернет
нужен DNS-проверкам (DoH-апстрим AdGuard) и серверу доступа (REALITY одалживает рукопожатие
настоящего сайта), оба пропускает `VIBEDPN_E2E_OFFLINE=1`. Почему устроено так — [knowledge/ci/e2eStand.md](../knowledge/ci/e2eStand.md).
