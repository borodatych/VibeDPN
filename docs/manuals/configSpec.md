...
- **Где лежит:** `/opt/vibedpn/config.yaml`. Пишет `vibedpn init`; править руками можно, после
  ...
  `upstreams.dpn.country`. Их правку руками `up` не видит: ядро подхватит её при следующей правке
  через CLI или панель либо после `vibedpn restart`. Проще менять их командами.
...
| `lists[].via` | `vps` \| `dpn` \| `wg` \| `tor` \| `direct` | — | канал всех доменов списка; аплинк должен быть включён в `upstreams` |
...
Раздел можно править руками (после правки — `vibedpn restart`: `up` ядро ради него не пересоздаёт), но проще командами
