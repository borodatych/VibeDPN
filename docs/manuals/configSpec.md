...
| Ключ | Тип | По умолчанию | Смысл |
|---|---|---|---|
| `enabled` | bool | `true` | выключить — VPS остаётся с тем, что настроил хостер |
| `ssh_ports` | список портов | `[22]` | `init` читает все `Port` из `sshd_config` и `sshd_config.d/`; пустой список — ошибка, чтобы не запереть себя |
