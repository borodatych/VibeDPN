...
| `domains[].via` | `vps` \| `dpn` \| `direct` | — | канал сайта; аплинк `vps`/`dpn` должен быть включён в `upstreams` |
...
| Ключ | Тип | По умолчанию | Смысл |
|---|---|---|---|
| `lists` | список | `[]` | готовые списки сайтов по URL для `mode: smart`; каждый домен списка идёт каналом списка |
| `lists[].url` | `http://` или `https://` URL | — | текстовый файл: по домену в строке, строки hosts-файла (`0.0.0.0 example.org`) или `\|\|example.org^`; `#` и `!` — комментарии; один URL дважды — ошибка |
| `lists[].via` | `vps` \| `dpn` \| `direct` | — | канал всех доменов списка; аплинк должен быть включён в `upstreams` |
