...
| `default_upstream` | `vps` \| `dpn` \| `wg-<имя>` | — (обязателен) | куда идёт трафик в `full`/`smart`. Если `mode` не `off`, аплинк должен быть включён в `upstreams`. У `home` — `dpn` или свой выход `wg-<имя>` из `upstreams.wg` |
...
| `domains[].via` | `vps` \| `dpn` \| `wg` \| `direct` | — | канал сайта; аплинк `vps`/`dpn` должен быть включён в `upstreams`, `wg` — свой выход из `upstreams.wg` |
...
| `lists[].via` | `vps` \| `dpn` \| `wg` \| `direct` | — | канал всех доменов списка; аплинк должен быть включён в `upstreams` |
