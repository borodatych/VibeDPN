...
То же обновление можно начать кнопкой «Обновить» на странице «Статус» панели: карточка «Обновление» показывает версию коробки (ветка, коммит, время коммита) и итог последнего обновления — с какого коммита на какой или почему не вышло.
Обновление выполняет хост, как и изменения из панели (раздел ниже): юнит `vibedpn-update-request.path` замечает запрос и запускает `vibedpn update --requested`.
Коробка при этом перезапускается, панель и выходы пропадают на минуту-другую, открытая страница перезагружается сама, когда обновление закончилось.
Версия появляется в карточке после первого `sudo vibedpn up` на новой версии: его же нужно запустить один раз, чтобы поставить юниты обновления.
