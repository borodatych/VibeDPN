...
скажут об этом одной строкой. Правили `config.yaml` — `vibedpn restart`: он пересобирает `.env`
(профили, порты, адрес), пересоздаёт изменившиеся контейнеры и перезапускает остальные, чтобы
они перечитали конфиг. `down` использует все профили, поэтому после смены роли не остаётся
чужих контейнеров.
