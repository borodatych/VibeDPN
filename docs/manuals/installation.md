...
проверку здоровья ядра. `down` использует все профили, поэтому после смены роли
...
Правили `wg_server` в `config.yaml` — `vibedpn restart` применит новый конфиг с тем же ключом.
