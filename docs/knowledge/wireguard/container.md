...
`0.0.0.0/0` метки не было бы вовсе, и туннель поднимался бы только по случайности: первый
handshake успевал уйти до установки правил, а дальше его пропускал `ct state established`
(проверено на старом варианте — сплит-туннель работал, но восстановиться после разрыва conntrack
уже не мог). Поэтому же таблица nft ставится **до** создания интерфейса: `oifname "wg0"`
