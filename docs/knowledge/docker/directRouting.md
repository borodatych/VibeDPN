...
**Дополнение (doctor, 2026-09-12):** Docker при старте включает `ip_forward` и ставит политику
DROP на пересылку («will prevent your Docker host from acting as a router», опция
`ip-forward-no-drop`) — транзит LAN → шлюзы в Stage 4 должен быть разрешён явно в DOCKER-USER,
`nat-unprotected` снимает только фильтр входа в бридж.
