...
## Раздающая нода без регистрации в перезапусках (2026-09-15)

С ретранслятором обе ноды проходят блокировку, но `myst-provider` всё равно перезапускается примерно раз в две минуты, и это уже экономика сети, а не фильтр провайдера:
- в логе повторяется `Identity … not registered` и `Waiting for registration success event`;
- её собственный `GET /services` на TequilAPI висит без ответа (замер с коробки: 20 с), тогда как `GET /identities` отвечает мгновенно;
- команда `service` при старте шлёт себе `POST /services`, не дожидается ответа и выходит: `failed to run service scraping: Post "http://0.0.0.0:4050/services": context deadline exceeded`.

Вывод: пока identity провайдера не зарегистрирована, публиковать предложение нечем, и нода уходит по таймауту.
Выход (`myst-consumer`) этим не затронут: 0 перезапусков, `/status` отдаёт identity без ошибки.
