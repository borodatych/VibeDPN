...
**Регистрация:** `POST /identities/{id}/register` (тело необязательно: `fee`, `beneficiary`, `referral_token`). Уже зарегистрирована — 200, в процессе — 422, иначе 202 и транзактор регистрирует в сети. Бесплатно — только если `canRegisterForFree`.
**Пути проверены на живой ноде 1.39.5 (2026-09-20) и записаны здесь ранее неверно:** право — `GET /identities/{id}/eligibility` → `{"eligible": bool}` (по `/transactor/identities/{id}/eligibility` нода отвечает 404), комиссия — `GET /v2/transactor/fees` → `{"current":{"registration":{"wei":"…","ether":"…"}}}` (по `/transactor/fees` ответа нет вовсе). На коробке владельца 20.09.2026: `eligible: false`, комиссия 0.10987 MYST.
