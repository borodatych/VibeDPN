...
## Цены: бесплатных нод нет, цену задаёт сеть

Проверено 2026-09-17 ночью публичными ответами discovery (с Mac, не с коробки):
- `GET https://discovery.mysterium.network/api/v4/proposals?service_type=wireguard` — 10 820 предложений
  WireGuard, поля цены в предложении нет (7.5 МБ ответа).
- Цены отдельно, `GET https://discovery.mysterium.network/api/v4/prices` (нода читает `prices` —
  `market/mysterium/mysterium_api.go`): `defaults.current` по типу адреса и услуге, плюс `per_country`
  на 249 стран. WireGuard: `residential` — 3.83 MYST за ГиБ, остальные — 1.72 MYST за ГиБ, в час
  0.00048 MYST у обоих. Нулевой цены нет ни в умолчаниях, ни по странам.
- Значит «подключиться бесплатно к бесплатным нодам» нельзя: цену назначает сеть, а не нода.
  Прежняя строка базы знаний про «0.1 MYST/ГиБ по умолчанию» устарела.
- Для самоокупаемой коробки заработок раздающей ноды должен покрывать 1.7–3.8 MYST за каждый ГиБ
  своего трафика.
