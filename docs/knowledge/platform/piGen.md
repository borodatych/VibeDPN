...
Тот же рецепт с `architecture:arm64` на том же amd64-хосте: 32 минуты (эмуляция внутри fakemachine, хосту ничего ставить не нужно), загрузочный тест под TCG — 5.5 минут, ssh отвечает через 55 с против 25 с под KVM.
Отсюда цена теста под эмуляцией: любые паузы в нём должны считаться от ускорителя, а не быть константой — 8 секунд до Ctrl+C хватало под KVM и приходились на импорт модулей CLI под TCG.
