...
Вероятная причина `0.0.0.0` — не AdGuard, а DNS среды стенда: 2026-09-25 найдено, что системный DNS машины временами отвечает на `dns.cloudflare.com` адресом `0.0.0.0` или испорченным пакетом, а перезапуск заставляет AdGuard заново разрешить имя апстрима. Вывод замера от этого не меняется: под нагрузкой перезапуск и без того терял запросы и сбрасывал кеш.
