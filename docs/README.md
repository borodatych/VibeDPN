...
├── uiVariants.md         # два варианта UI на start0 (N100 и Raspberry Pi): что режется, память, CI, выбор
├── troubleshooting.md    # что делать, когда не работает: симптом → чем проверить → что это значит
├── decisions.md          # решения на развилках, принятые без владельца: что выбрано и что перепроверить
...
│   ├── languageFileSpec.md # формат языкового файла панели: скормить модели и получить перевод
│   ├── wgUplink.md       # свой выход по готовому файлу WireGuard: рецепт без криптовалюты
│   ├── multiVps.md       # несколько VPS: второй подхватывает, когда первый молчит — цепочка routing.fallback
...
│   ├── telegramBot.md    # Telegram-бот коробки: тревоги и недельный отчёт, привязка чата, настройки
...
└── knowledge/            # база знаний: грабли и проверенные факты с URL
    ...
    ├── myst/  adguard/  compose/  docker/  wireguard/  tor/  xray/  ddns/  telegram/  platform/  linux/  security/  ci/  frontend/
    ├── python/
    └── process/
```
