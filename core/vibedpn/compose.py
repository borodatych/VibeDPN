...
from vibedpn.engine.router import country_uplinks
...
def compose_files(box_dir: Path) -> list[Path]:
    ...
    files += [
        path for path in (box_dir / COUNTRIES_FILE, box_dir / OVERRIDE_FILE) if path.is_file()
    ]
    return files
...
def render_countries(config: Config) -> str:
    """One consumer per exit country of routing.domains, extending `myst-consumer` of compose.yaml:
    only its address in the gateway network and its data directory differ (decision 20)."""
    lines = [
        "# Generated by vibedpn from routing.domains at every `up`/`restart`; do not edit.",
